...
功能:
- 扫描指定知识库目录下的所有 Markdown (.md) 和文本 (.txt) 文档
...
知识库目录优先级:
- 环境变量 `RAG_KB_DIR`（绝对路径或相对 `project_root`）
- 默认目录 `<project_root>/rag/vulnerabilities`
"""
...
def kb_root_dir(root: str) -> str:
    # 优先使用环境变量指定的知识库目录。
    env_dir = (os.getenv("RAG_KB_DIR") or "").strip()
    if env_dir:
        if os.path.isabs(env_dir):
            return env_dir
        return os.path.join(root, env_dir)

    # 默认使用本地漏洞知识库目录。
    return os.path.join(root, "rag", "vulnerabilities")
...
def main():
    ...
    root = project_root()
    kb_dir = kb_root_dir(root)
    print("RAG_KB_DIR =", kb_dir)
