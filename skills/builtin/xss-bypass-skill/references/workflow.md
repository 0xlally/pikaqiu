...
如果 payload 触发了 challenge 文案、图片、按钮、跳转、cookie、session 或响应长度的明显状态变化，先暂停字典和标签扩散。把这个 payload 当作状态机入口，按三类信号拆开验证：

- 反射信号：原始响应里输入是否还存在，出现在 HTML 文本、属性、JS、URL 还是被后端消费。
- 状态信号：状态变化是否稳定复现，是否只是“进入下一阶段/开始游戏”，还是已出现 flag 或题目定义的成功文本。
- 执行信号：浏览器是否创建节点、触发事件、产生 dialog/console/DOM 变化、资源请求、同源状态变化或 bot callback。

只在这三类信号缺口明确后继续生成下一个最小探针。例如 `<x>`、`<img ...>`、`<svg/...>` 都返回同一个 “Let the game begin” 时，不要继续比较更多标签；先比较 `<x>`、`<x></x>`、`<x>x</x>`、`<x attr=1>` 的 raw HTML、最终 DOM 和状态文本，确认后端到底把“看起来像标签”当通关条件、过滤条件还是可执行 HTML。
...
边界/自定义标签只回答“过滤器边界在哪里”，不是默认解题方向。若任意简单开标签都会触发同一状态，优先分析状态机，不要把 `<z>`、`<zz>`、`<x autofocus...>` 当成独立候选继续刷。

## 探测超时纪律

- 不用 `--max-time 1` 或单次网络抖动给 payload 下结论；基线请求先用可复现的实用超时。
- 管道命令会掩盖失败码时，使用 `set -o pipefail`，或用 Python 捕获异常并打印每个 payload 的状态。
- 批量探测每批 3-5 个 payload，逐项 `flush` 输出；外层总超时必须大于“单项 timeout * payload 数”，否则超时结果不可归因。
- 一个 payload 变慢时单独复测；不要把混合 payload 批次的整体超时当过滤规则。
