    ...
    *,
    ...
    sections = [
        ...
        (
            ...
            "需要联网搜索时用 `web_search`；漏洞技巧和本地经验可用 `knowledge_search`。\n"
            "除非当前假设已被目标行为卡住，否则先用 bash_exec/python_exec 直接验证；联网搜索和知识库只用于补充公开资料、确认版本/机制或在没有思路时破局。web_search 每次只能提交一个聚焦 query。\n"
        ...
        (
            ...
            "优先自己基于目标响应、源码/页面、状态码、差异行为构造验证；不要把 `web_search`/`knowledge_search` 当作默认第一步。确实需要搜索时，`web_search` 一次只传一个 query。\n"
            "- **web_search**: 联网搜索工具\n"
        ...
        (
            "## 沙箱工具速查\n"
            "- **Web发现/参数/漏洞**：`curl`、`httpx`、`ffuf`、`arjun`、`nuclei`、`sqlmap`、`wpscan`、`searchsploit`。\n"
            "- **端口/内网探测**：`nmap`、`fscan`。\n"
